...
{
	weaponSelected = objInventory.selected;
}
...
if (!global.forms.lunarkid) {
	...
	if (jump_mod.high == 1) {
		...
		jump_height[1] = 7.5/speedDivider;
	}
	#endregion
	...
	if (dir != 0) {
		...
		if(cantChangeSprite <= 0)
		{
			if(isCrouching = false)
			{
				...
				{
					player_sprite("IdleUp");
				}
			}else
			{
				player_sprite("Crouching");
			}
		}
	}
	
	if(isCrouching == false && isSliding == false)
	{
		if(keyboard_check(global.controls.up))
		{
			aimingUp = 1;
		}else if(keyboard_check(global.controls.down) && vspd != 0)
		{
			aimingUp = -1;
		}else if(!keyboard_check(global.controls.up) && !(keyboard_check(global.controls.down)) || vspd = 0 && aimingUp != 1)
		{
			aimingUp = 0;
		}
	...
	if (!on_platform) {
		if(cantChangeSprite <= 0)
		{
			if (vspd * global.grav < -0.05) {
				...
				{
					player_sprite("JumpUp");
				}else
				{
					player_sprite("JumpDown");
				}
	...
	if (!frozen || instance_place_check(x, y, objWeirdWater, tangible_collision) != null) {
		...
		{
			if(global.shootOrJump = true)
			{
				if (is_pressed(global.controls.shoot) && slideCounter < 0) {
					slideCounter = 99;
					canSlideShoot = false;
					isSliding = true;
					audio_play_sound(sndSlide, 0, 0);
					alarm[0] = 15;
					alarm[1] = 1;
				}				
			}else
			{
				if (is_pressed(global.controls.jump) && slideCounter < 0) {
					slideCounter = 99;
					canSlideShoot = false;
					isSliding = true;
					audio_play_sound(sndSlide, 0, 0);
		...
		if(hspd != 0 && vspd = 0)
		{
			if(keyboard_check(global.controls.left) || keyboard_check(global.controls.right))
			{
				if(trailCounter < 0)
				{
					trailCounter = 6;
					instance_create_layer(x, y, layer, objWalkTrail);
				}
			}
		}
		...
		if(keyboard_check_direct(global.controls.down) && vspd = 0 && isSliding = false)
		{
			isCrouching = true;
		}else if(!keyboard_check_direct(global.controls.down) || vspd != 0 || isSliding = true)
		{
			isCrouching = false;
		}
		#endregion
...
if (!frozen) {
	if (keyboard_check(global.controls.shoot) && !global.controls_lock.shoot && canSlideShoot && reload < 0) {
		...
		if(weaponSelected = 1)
		{
			instance_create_layer(x, y, layer, objShotgunBlast);
		}
		...
		if(weaponSelected = 2)
		{
			...
			cantChangeSprite = 15;
			player_sprite("Sword");
			instance_create_layer(x, y, layer, objPlayerSword);
		}
		
		if(weaponSelected = 3)
		{
			if(instance_number(objPlayerBomb) = 0)
			{
				image_index = 0;
				cantChangeSprite = 15;
				player_sprite("Throw");
				instance_create_layer(x, y, layer, objPlayerBombSpawner);
			}else
			{
				image_index = 0;
				cantChangeSprite = 25;
				player_sprite("Detonate");
				with(objPlayerBomb)
				{
					explode();
				}
...
if(weaponSelected != 3)
{
	if(instance_exists(objPlayerBomb))
	{
		with(objPlayerBomb)
		{
			explode();
