...
function player_shoot() {
	var bullet_max = (global.slowshot) ? 10 : 8;
	...
	if (instance_number(objBullet) < bullet_max) {
		if(objPlayer.xscale = 1 && objPlayer.aimingUp = 0)
		{
			instance_create_layer(x-5, y-1, "Player", bullet_object);
			audio_play_sound(shoot_sound, 0, false);
		}else if(objPlayer.xscale = -1 && objPlayer.aimingUp = 0)
		{
			instance_create_layer(x+5, y-1, "Player", bullet_object);
			audio_play_sound(shoot_sound, 0, false);		
		}
		
		if(objPlayer.aimingUp = 1)
		{
			if(objPlayer.xscale = 1)
			{
				instance_create_layer(x+6, y+5, "Player", bullet_object);
				audio_play_sound(shoot_sound, 0, false);		
			}else
			{
				instance_create_layer(x-6, y+5, "Player", bullet_object);
		...
		if(objPlayer.aimingUp = -1)
		{
			instance_create_layer(x, y+1, "Player", bullet_object);
			audio_play_sound(shoot_sound, 0, false);		
		}
	}
}
